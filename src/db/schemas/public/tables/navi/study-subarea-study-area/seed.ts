import { stringToNumber } from '../../../../../utils/coercion';
import { Seed } from '../../../../../utils/seed';
import { timestampsDefaultValues } from '../../../../../utils/timestamps';

import { studySubareaStudyAreaNavi as t } from '.';

class StudySubareaStudyAreaNaviSeed extends Seed<
  typeof t,
  typeof t.$inferSelect
> {
  protected async transform(
    row: Record<keyof typeof t.$inferSelect, string>,
  ): Promise<typeof t.$inferSelect> {
    return new Promise((resolve) => {
      resolve({
        ...row,
        ...timestampsDefaultValues,
        sort: stringToNumber(row.sort),
        studySubareaStudyAreaId: stringToNumber(row.studySubareaStudyAreaId),
      });
    });
  }
}

const studySubareaStudyAreaNavi = new StudySubareaStudyAreaNaviSeed(t);

export { studySubareaStudyAreaNavi };
